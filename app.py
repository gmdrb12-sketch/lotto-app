...
# --- 슈퍼KTS 실시간 스크래핑 (가장 안정적인 방식) ---
@st.cache_data(ttl=3600)
def fetch_data_superkts():
...
def generate_strategy_combinations(strategy_type, w, c_w, top_hot):
    for _ in range(1000): # 필터 통과할 때까지 반복
        if strategy_type == "HOT":
            game = sorted(random.sample(top_hot, 6)) # 상위 핫 넘버 중 집중 투하
        elif strategy_type == "COLD":
            pool = list(range(1, 46))
            game = sorted(random.choices(pool, weights=c_w, k=6))
        elif strategy_type == "BALANCED":
            evens = 0
            while evens != 3: # 완벽한 3:3 홀짝 비율 강제
...
st.subheader("슈퍼KTS 실시간 분석 및 통계 조합 엔진")

with st.spinner("슈퍼KTS에서 실시간 데이터를 수집 중입니다..."):
    recent_data, err = fetch_data_superkts()
